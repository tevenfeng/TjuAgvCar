...
import numpy as np
from SeparableAtrousCnn import SeparableAtrousCnn
...
if __name__ == '__main__':
    path = r'/Users/tevenfeng/Coding/python/backup/data/305/19-04-04-02'  # 19-04-04-02 includes processed images of 305
    ...
    for i in range(num_epochs):
        ...
        for b in range(0, batches_per_epoch):
            ...
            # ------------------------
            train_step.run(
                feed_dict={model.x: batch[0], model.y_: batch[1],
                           model.keep_prob_fc1: 0.2, model.keep_prob_fc2: 0.2,
                           model.keep_prob_fc3: 0.2, model.keep_prob_fc4: 0.2})
        ...
        if i % 5 == 0:
            ...
            for b in range(0, len(test_x), batch_size):
                ...
                test_error_ = model.loss.eval(feed_dict={model.x: batch[0], model.y_: batch[1],
                                                         model.keep_prob_fc1: 0, model.keep_prob_fc2: 0,
                                                         model.keep_prob_fc3: 0, model.keep_prob_fc4: 0})
                # -----------------------
                test_error += test_error_
            test_error /= len(test_x) / batch_size
            test_accuracy = 1.0 - test_error
            print("test accuracy %g" % test_accuracy)
    filename = saver.save(sess, r'/Users/tevenfeng/Coding/python/TrainCNN/model/SeparableAtrousCnn/model.ckpt')
